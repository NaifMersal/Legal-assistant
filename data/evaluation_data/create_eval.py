...
2. **الأسئلة**: صِغ أسئلة قانونية واضحة ومحددة تستند مباشرة إلى النص الموجود في الـ context.  
   - تجنّب الأسئلة المباشرة مثل: "وفقًا للمادة رقم..." أو "بحسب المادة..." أو أي سؤال يذكر رقم المادة صراحة.  
   - بدلاً من ذلك، اطرح السؤال بصيغة عامة تعتمد على المعنى والمضمون، وليس على رقم المادة.  
   - تجنّب الأسئلة المفتوحة جداً أو التي تتطلب معرفة خارجية.  
            ...
            while q_generated_for_law < total_q_needed:
                ...
                try:
                    ...
                    for qa in qa_output.qa_pairs:
                        # Check if all reference IDs are in selected_articles
                        if not set(qa.references_ids).issubset(set(selected_articles)):
                            logger.warning(f"    Skipping QA pair - reference IDs {qa.references_ids} not subset of selected articles {selected_articles}")
                            continue
